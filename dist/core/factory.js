...
import { classify, pickAgentForKey } from './router.js';
...
export function createSystemContext() {
    ...
    for (const [name, cls] of Object.entries(AGENT_CLASSES)) {
        ...
        const agent = new cls(config, llm, bus, agentReg, agentSkills);
        agentReg.register(createDelegateTool(agents, agent));
        agents[name] = agent;
    }
    return {
        config,
...
export function isThinContent(content) {
    if (!content)
        return true;
    ...
    if (PLACEHOLDER_PATTERNS.has(bare))
        return true;
...
function looksObviouslyComplete(results) {
    ...
    for (const r of results) {
        ...
        if (body.length < 400)
            return false;
        if (RESULT_FAILURE_MARKERS.some((m) => body.includes(m)))
            return false;
    }
    return true;
}
/** Run agent.executeTask with bounded retries on failure / thin content. */
async function executeWithRetry(agent, task, maxAttempts) {
    let lastResult = null;
    let lastErr = null;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (attempt < maxAttempts)
            await sleep(Math.min(0.5 * 2 ** (attempt - 1), 2.0) * 1000);
    }
    task.description = originalDescription;
    if (lastResult)
        return lastResult;
...
async function executePending(pending, agentMap, results, resultsById, fullContentsById, completed, resultTruncate, maxTaskRetries) {
...
async function judgeGoalAchievement(snow, goal, results) {
    const bullets = results
        ...
        .join('\n');
    const prompt = `你是一名极严格的项目验收员。验证 sub-task 真的产出了**可验证的交付物**，而不是仅声称完成。\n\n` +
    ...
    let raw;
    try {
        raw = await snow.chatOneshot(prompt);
    }
    catch (exc) {
...
function hasCycle(t, tasks, path) {
    if (path.has(t.id))
        return true;
    path.add(t.id);
    for (const depId of t.allDeps) {
        const depTask = tasks.find((x) => x.id === depId);
        if (depTask && hasCycle(depTask, tasks, new Set(path)))
            return true;
    }
    return false;
}
 ...
 */
export async function runOrchestration(goal, agentMap, snow, opts = {}) {
    ...
    let tasks = matched
        ...
        : await snow.orchestrate(goal);
    const completed = new Set();
    const results = [];
    const resultsById = new Map();
    const fullContentsById = new Map();
    let replanRound = 0;
    const filterCycles = (cand) => {
        const kept = [];
        for (const t of cand) {
    ...
    while (true) {
        await executePending(pending, agentMap, results, resultsById, fullContentsById, completed, resultTruncate, maxTaskRetries);
        pending = [];
        ...
        let extraTasks;
        try {
            extraTasks = await snow.replanForMissing(goal, results, missing, new Set(tasks.map((t) => t.id)));
        }
        catch (exc) {
    ...
    if (!results.length) {
        ...
        summary = await snow.chatOneshot(summaryPrompt);
    }
...
export async function orchestrateTask(ctx, goal) {
    ...
    if ((mode === 'direct' || mode === 'single') && !matchPipeline(goal)) {
        ...
        if (!agent)
            return { mode, result: 'No agent available.' };
        await agent.init();
    ...
    if (!snow)
        return { mode: 'orchestrate', result: 'Snow agent not available.' };
    for (const agent of Object.values(ctx.agentMap))
        await agent.init();
