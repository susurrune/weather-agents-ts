...
export class Skill {
    ...
    static fromMarkdown(path) {
        ...
        if (Array.isArray(allowedRaw)) {
            const list = allowedRaw.filter((t) => typeof t === 'string');
...
function normalizeClaudeToolName(raw) {
    let s = raw.trim();
    if (!s)
        return s;
...
function extractSkillHead(body, maxChars) {
    const out = [];
    let charCount = 0;
    let h2Count = 0;
    for (const line of body.split('\n')) {
        ...
        if (out.length && charCount + line.length + 1 > maxChars)
            break;
        out.push(line);
