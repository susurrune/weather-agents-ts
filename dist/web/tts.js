...
export class DoubaoTTS {
    ...
    requestBody(text) {
        ...
        if (loudness !== 0)
            audioParams.loudness_rate = loudness;
        return {
            user: { uid: 'wa_voice' },
            req_params: {
                text,
...
function mapRate(value, srcMin, srcMax, dstMin, dstMax) {
    if (value <= 0)
        return dstMin;
