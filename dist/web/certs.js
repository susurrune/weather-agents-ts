...
export function detectAllLanIps() {
    const ips = [];
    try {
    ...
    try {
        ...
        if (addr && addr.address && !ips.includes(addr.address))
...
export function ensureSelfSignedCert(ips) {
    if (!ips.length)
        ips = ['127.0.0.1'];
    ...
    try {
        execSync(`openssl req -x509 -newkey rsa:2048 -nodes -keyout "${keyPath}" -out "${certPath}" ` +
            `-days 3650 -subj "/CN=weather-agents-voice" -addext "subjectAltName=${sanList}"`, { stdio: 'pipe', timeout: 30000 });
    }
    catch {
