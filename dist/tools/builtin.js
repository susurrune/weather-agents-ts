...
import { readFileSync, writeFileSync, existsSync, mkdirSync, statSync, readdirSync, renameSync, copyFileSync, unlinkSync, rmdirSync, realpathSync, } from 'node:fs';
...
export function isProtectedPath(path) {
    ...
    for (const cand of candidates) {
        ...
        if (WRITE_PROTECT_EXACT_POSIX.has(cand) || WRITE_PROTECT_EXACT_POSIX.has(stripped))
            return true;
        // Drive root: "c:" or "c:/"
...
function truncate(text, limit, label = 'output') {
...
export function validateUrl(url) {
    ...
    if (octets) {
        ...
        if (o1 === 192 && o2 === 168)
            return `Error: refusing to reach private/loopback IP ${host}`;
        if (o1 === 169 && o2 === 254)
            return `Error: refusing to reach link-local IP ${host}`;
        if (o1 >= 224)
            return `Error: refusing to reach multicast/reserved IP ${host}`;
    }
    // IPv6 loopback
    if (host === '::1' || host === '0:0:0:0:0:0:0:1')
        return `Error: refusing to reach IPv6 loopback`;
    return null;
}
/**
...
function parseIpv4Octets(host) {
    ...
    for (const p of parts) {
        ...
        if (/^0x[0-9a-f]+$/i.test(p))
            ...
            return null; // contains non-numeric → hostname, not an IP literal
...
async function httpPost(url, data = '') {
    ...
    if (err)
        return err;
    try {
        const headers = { 'User-Agent': 'WeatherAgents/1.0' };
...
// ── File tools ────────────────────────────────────────────────────────────────
async function _readFile(path, offset = 0, limit = 0) {
    ...
    try {
        ...
        if (e.code === 'EACCES')
            return `Error: Permission denied: ${p}`;
...
async function _listDirectory(path = '.', includeHidden = false) {
    const p = expandPath(path);
    try {
...
async function _listRecursive(path = '.', maxDepth = 4) {
    const p = expandPath(path);
    const lines = [];
    try {
...
async function _glob(pattern, directory = '.') {
...
async function _grep(pattern, directory = '.', caseSensitive = false) {
    const dir = expandPath(directory);
...
// ── Shell exec ────────────────────────────────────────────────────────────────
async function _shellExec(command, timeout = 30, cwd = '') {
    ...
    if (BLOCKED_COMMANDS.has(base))
        return `Blocked: '${base}' is not allowed for security reasons.`;
    for (const a of args.slice(1)) {
        if (/[;&|`$({]/.test(a))
            return `Blocked: shell metacharacter in argument: '${a}'`;
    }
    const workDir = cwd ? expandPath(cwd) : undefined;
    if (cwd && !existsSync(workDir))
...
async function _fetchWebPage(url, extractText = true) {
    ...
    try {
        ...
        if (!extractText)
            return truncate(html, 20000, 'html');
        // Strip HTML tags
        const text = html
...
// ── Registration ──────────────────────────────────────────────────────────────
export function registerBuiltinTools(reg) {
    ...
    const tools = [
        ...
        new Tool({
            ...
            handler: (a) => _editFile(String(a.path), String(a.old_text), String(a.new_text), Number(a.count) || 1),
            dangerous: true,
        }),
        new Tool({
            name: 'list_directory',
        ...
        new Tool({
            ...
            handler: (a) => _grep(String(a.pattern), String(a.directory ?? '.'), Boolean(a.case_sensitive)),
        }),
        new Tool({
            name: 'shell_exec',
