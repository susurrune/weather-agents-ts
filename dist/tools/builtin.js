...
import { readFileSync, writeFileSync, existsSync, mkdirSync, statSync, readdirSync, renameSync, copyFileSync, unlinkSync, rmdirSync, } from 'node:fs';
...
export function isProtectedPath(path) {
    ...
    for (const resolved of candidates) {
        ...
        if (WRITE_PROTECT_EXACT.has(normalized))
            return true;
        ...
        for (const root of WRITE_PROTECT_ROOTS) {
            ...
            if (normalized === r || normalized.startsWith(r + sep.toLowerCase()))
                return true;
        }
    }
    return false;
