...
import { type BaseAgent, Task } from '../core/agent.js';
...
function buildSharedContext(callingAgent: BaseAgent | null, context: string): string {
  const parts: string[] = [];
  ...
  if (callingAgent) {
    ...
    if (nonSystem.length) {
      ...
      const msgText = ctxMsgs
        ...
        .join('\n');
      if (msgText) parts.push(`Calling agent context:\n${msgText}`);
    }
...
export function createDelegateTool(
  ...
  callingAgent: BaseAgent | null = null,
): Tool {
  const handle = async (args: Record<string, any>): Promise<string> => {
    ...
    const target = agentMap[agent as string];
    if (!target) {
    ...
    if (callingAgent && (agent as string) === callingAgent.name) {
    ...
    return delegationDepth.run(store + 1, async () => {
      try {
        await target.init();
        const sharedCtx = buildSharedContext(callingAgent, context as string);
        const taskObj = new Task({
          id: `dlg-${nextId()}`,
          description: task as string,
          assignedTo: agent as string,
          metadata: sharedCtx ? { context: sharedCtx } : {},
        });

        log.info('delegation_start', {
          target: agent,
          task: (task as string).slice(0, 120),
          depth: store + 1,
        });

        const result = await target.executeTask(taskObj);
        if (target.state === AgentState.ERROR) {
          await (target as any).setState(AgentState.IDLE);
        }
  ...
  return new Tool({
    name: 'delegate_to',
    description:
      "Delegate a task to a specialist agent and receive the result. Use this when a task would benefit from another agent's expertise. Available agents and their specialties:\n" +
