...
function writeYaml(path: string, data: Record<string, any>): void {
...
export function saveUserCfg(data: Record<string, any>): void {
...
export function formatModelsForDisplay(catalog: Record<string, ModelEntry[]>): string {
  ...
  for (const [provider, models] of Object.entries(catalog)) {
    ...
    for (const m of models) {
      ...
      if (m.input_cost_per_1k) costParts.push(`$${m.input_cost_per_1k.toFixed(4)}/1k in`);
      if (m.output_cost_per_1k) costParts.push(`$${m.output_cost_per_1k.toFixed(4)}/1k out`);
...
export function getModelContextWindow(modelName: string): number {
  if (typeof modelName !== 'string') return 128000;
...
let _providerCache: Record<string, Record<string, any>> | null = null;

export function loadProviderCatalog(): Record<string, Record<string, any>> {
  ...
  if (existsSync(bundled)) {
    ...
    for (const [k, v] of Object.entries(data)) {
      if (v && typeof v === 'object' && !Array.isArray(v)) {
        catalog[k] = { ...(v as Record<string, any>) };
      }
    }
  }
  ...
  if (existsSync(userPath)) {
    ...
    for (const [k, v] of Object.entries(userData)) {
      if (v && typeof v === 'object' && !Array.isArray(v)) {
        catalog[k] = { ...(catalog[k] ?? {}), ...(v as Record<string, any>) };
      }
    }
  }
  ...
  if (entry === undefined) {
    for (const v of Object.values(cat)) {
      ...
      if (
        ...
        aliases.map((a: string) => a.toLowerCase()).includes(provider.toLowerCase())
      ) {
        entry = v;
        break;
      }
...
export function resolveProviderAlias(name: string): string {
  if (!name) return name;
  const cat = loadProviderCatalog();
  const lower = name.toLowerCase();
  if (lower in cat) return lower;
  for (const [k, v] of Object.entries(cat)) {
    const aliases = v.aliases;
    if (Array.isArray(aliases) && aliases.map((a: string) => a.toLowerCase()).includes(lower)) {
      return k;
    }
  }
  return lower;
...
function syncApiKeysToEnv(apiKeys: Record<string, string>): void {
...
function loadConfigUncached(): AppConfig {
  ...
  if (agents) {
    for (const name of AGENT_NAMES) {
      ...
      if (agentCfg) {
        if (agentCfg.model) cfg.agents[name].model = agentCfg.model;
        if (agentCfg.specialty) cfg.agents[name].specialty = agentCfg.specialty;
  ...
  if (cliCfg) {
    ...
    cfg.cli.circuitFailureThreshold = Math.trunc(
      Number(cliCfg.circuit_failure_threshold ?? cfg.cli.circuitFailureThreshold),
    );
    cfg.cli.circuitRecoveryTimeout = Number(
      cliCfg.circuit_recovery_timeout ?? cfg.cli.circuitRecoveryTimeout,
    );
    cfg.cli.rateLimitMaxCalls = Math.trunc(
      Number(cliCfg.rate_limit_max_calls ?? cfg.cli.rateLimitMaxCalls),
    );
...
export function setConfig(key: string, value: string): [boolean, string] {
  ...
  if (parts.length === 2 && parts[0] === 'cli' && parts[1] === 'default_agent') {
    if (!(AGENT_NAMES as readonly string[]).includes(value.toLowerCase())) {
  ...
  if ((SIMPLE_LLM_KEYS as readonly string[]).includes(key)) {
    let typedVal: string | number = value;
    if (key === 'temperature') {
      typedVal = Number(value);
      if (Number.isNaN(typedVal)) return [false, `invalid value for ${key}: '${value}'`];
      if (!(typedVal >= 0.0 && typedVal <= 2.0))
        return [false, 'temperature must be in [0.0, 2.0]'];
    } else if (key === 'max_tokens') {
      typedVal = Number(value);
      if (!Number.isInteger(typedVal)) return [false, `invalid value for ${key}: '${value}'`];
...
export function deleteConfig(key: string): [boolean, string] {
  const path = join(USER_CONFIG_DIR, 'config.yaml');
  const data = loadYaml(path);
  ...
  const popKey = (obj: Record<string, any> | undefined, k: string): boolean => {
    if (obj && k in obj) {
      delete obj[k];
      return true;
    }
  ...
  if (parts.length === 2 && parts[0] === 'api_key') {
    ...
    if (removed) writeYaml(path, data);
