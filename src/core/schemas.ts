 ...
 */
export function parseSchema<S extends z.ZodTypeAny>(raw: string, schema: S): z.output<S> {
...
export function parseTaskPlan(raw: string): TaskPlan | null {
  try {
    ...
    if (e instanceof SchemaValidationError) return null;
    throw e;
  }
}

/** Parse fact-extraction output into structured facts, or null. */
export function parseFacts(raw: string): ExtractionResult | null {
  try {
