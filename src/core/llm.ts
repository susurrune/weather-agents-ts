...
import { type AppConfig, loadProviderCatalog, getProviderEnvVar } from './config.js';
...
function buildKnownProviders(): Set<string> {
  try {
    const cat = loadProviderCatalog();
    const known = new Set<string>();
    for (const [provider, entry] of Object.entries(cat)) {
      known.add(provider.toLowerCase());
      for (const alias of (entry.aliases as string[]) ?? []) {
        if (typeof alias === 'string') known.add(alias.toLowerCase());
      }
    }
...
export function splitProvider(model: string): [string | null, string] {
  if (!model.includes('/')) return [null, model];
  const idx = model.indexOf('/');
  const head = model.slice(0, idx);
  const tail = model.slice(idx + 1);
  if (KNOWN_PROVIDERS.has(head.toLowerCase())) return [head.toLowerCase(), tail];
  return [null, model];
}
...
export function isAnthropicModel(model: string): boolean {
  ...
  if (lowered.startsWith('anthropic/') || lowered.startsWith('claude')) return true;
...
function buildProviderEnvMap(): Record<string, string> {
  try {
    ...
    for (const [provider, entry] of Object.entries(cat)) {
      ...
      if (typeof env === 'string' && env) out[provider.toLowerCase()] = env;
      for (const alias of (entry.aliases as string[]) ?? []) {
...
export function formatUserFacingError(model: string, err: unknown): string {
...
const MODEL_COST_ESTIMATES: Record<string, [number, number]> = {
  'gpt-4o': [0.0025, 0.01],
...
export function isTransientError(exc: unknown): boolean {
  const e = exc as { statusCode?: number; status?: number; name?: string };
...
// ── Completion backend (the provider seam) ──────────────────────────────────
...
export class LLMClient {
  ...
  private usageStats: Record<string, UsageStat> = {};
  private totalCost = 0;
  ...
  constructor(
    ...
    opts: { costLimit?: number | null; backend?: CompletionBackend } = {},
  ) {
    this.config = config;
  ...
  private getModel(agentName: string | null = null): string {
    if (agentName) {
      const agentCfg = (this.config.agents as Record<string, { model: string | null }>)[agentName];
      if (agentCfg && agentCfg.model) return agentCfg.model;
    }
  ...
  private hasKeyForModel(model: string): boolean {
    ...
    if (provider === null) return true; // can't determine; don't skip
  ...
  private errorResponse(model: string, content: string): LLMResponse {
    return {
      content,
      toolCalls: [],
      model,
  ...
  async *streamWithTools(
    ...
    const ov = overrides ?? {};
    const primaryModel: string = typeof ov.model === 'string' ? ov.model : this.getModel(agentName);
    const fallbackModels = (FALLBACK_CHAINS[primaryModel] ?? []).filter((m) =>
      this.hasKeyForModel(m),
    );
    ...
    let iterator: AsyncIterable<RawStreamChunk> | null = null;
    ...
    const elapsed = performance.now() - start;
    const promptTokens =
      backendPromptTokens || Math.max(1, Math.floor(JSON.stringify(messages).length / 4));
    const completionTokens =
      backendCompletionTokens || Math.max(1, Math.floor(fullContent.length / 4));
    this.trackUsage(agentName, model, promptTokens, completionTokens);
    logEvent(log, 'llm_stream', {
      model,
      agent: agentName,
  ...
  private streamEvent(
    type: StreamEventType,
    parts: Partial<Omit<StreamEvent, 'type'>> = {},
  ): StreamEvent {
    return {
      type,
      text: parts.text ?? '',
 ...
 */
export function toCoreMessages(messages: Array<Record<string, any>>): Array<Record<string, any>> {
...
export class AiSdkBackend implements CompletionBackend {
  private async makeModel(req: CompletionRequest): Promise<any> {
    ...
    try {
      ...
      if (typeof bu === 'string' && bu) baseURL = bu;
    } catch {
  ...
  private async buildTools(
    ...
    for (const s of schemas) {
      out[s.function.name] = tool({
        ...
        parameters: jsonSchema(s.function.parameters as any),
  ...
  async complete(req: CompletionRequest): Promise<RawCompletion> {
    ...
    const result = await generateText({
      model,
      messages: toCoreMessages(req.messages) as any,
    ...
    return {
      content: result.text ?? '',
      toolCalls,
      reasoningContent: (result as any).reasoning ?? null,
  ...
  async *stream(req: CompletionRequest): AsyncIterable<RawStreamChunk> {
