...
function tokenize(text: string): Set<string> {
  ...
  for (const m of (text || '').matchAll(TOKEN_RE)) {
    ...
    if (t.length >= 2 && !STOPWORDS.has(lower)) out.add(lower);
  }
  return out;
}
...
function scoreTool(tool: Tool, queryTokens: Set<string>): number {
  ...
  for (const qt of queryTokens) {
    if (nameTokens.has(qt)) score += 5;
    else if (nameLower.includes(qt)) score += 3;
  }
...
export function selectRelevantTools(
  ...
  query: string,
  opts: { topK?: number; mustInclude?: Set<string> } = {},
): string[] {
  const topK = opts.topK ?? 12;
  const must = new Set<string>([...(opts.mustInclude ?? []), ...ALWAYS_INCLUDE]);
