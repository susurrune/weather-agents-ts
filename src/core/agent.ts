...
import { EventType, type Event, type MessageBus, makeEvent } from './bus.js';
import { TASK_DONE_SENTINEL } from './constants.js';
import { type AppConfig, getModelContextWindow } from './config.js';
import { type LLMClient, type LLMResponse } from './llm.js';
import { getLogger } from './logger.js';
import { type Message } from './memory.js';
import { Memory } from './memory.js';
import { type Skill, SkillRegistry } from './skill.js';
import { Tool, type ToolRegistry } from './tool.js';
...
export class Task {
  ...
  get allDeps(): string[] {
    ...
    if (this.parentId && !deps.includes(this.parentId)) deps.push(this.parentId);
    return deps;
  }
}
...
const TOOL_FAILURE_MARKERS: readonly string[] = [
  'no results found',
  'no matches for',
  'file not found',
  'directory not found',
...
const FILE_PRODUCING_TOOLS: Record<string, readonly string[]> = {
  write_file: ['path'],
  edit_file: ['path'],
...
function parseToolArgs(raw: string): Record<string, any> | null {
  ...
  cleaned = cleaned.replace(RE_UNQUOTED_STRING, (m, g1, g2, g3) => {
    if (g2 === 'null' || g2 === 'true' || g2 === 'false') return m;
    if (/^-?\d/.test(g2) && /^[\d.]+$/.test(g2)) return m;
    if (g2.startsWith('"') || g2.startsWith('{') || g2.startsWith('[')) return m;
    return `${g1}"${g2}"${g3}`;
  });
  // 11. Try parse
  try {
...
function formatArgsParseError(toolName: string, rawArgs: string): string {
...
function toolCallSignature(toolName: string, args: Record<string, any> | null): string {
  const a = args ?? {};
  const s = (key: string, n?: number): string => {
    const v = a[key];
  ...
  if (toolName === 'delegate_to') return `delegate_to:${s('agent')}`;
  return toolName;
}
...
function suggestToolNames(missing: string, registry: ToolRegistry, maxN = 3): string[] {
  ...
  if (!chunks.length && missingLower.length >= 3) chunks = [missingLower];
  const nameScored: Array<[number, string]> = [];
  const descScored: Array<[number, string]> = [];
  for (const n of allNames) {
    let ns = 0;
    for (const chunk of chunks) {
      if (n.toLowerCase().includes(chunk)) ns += 2;
    }
    for (const chunk of n.toLowerCase().split('_')) {
      if (chunk.length >= 3 && missingLower.includes(chunk)) ns += 1;
    }
    if (ns > 0) nameScored.push([ns, n]);
    const tool = registry.get(n);
    if (!tool) continue;
    const desc = tool.description.toLowerCase();
    const ds = chunks.filter((c) => desc.includes(c)).length;
    if (ds > 0) descScored.push([ds, n]);
  }
...
function toolStatusLabel(name: string, args: Record<string, any>): string {
...
function synthesizeDelegationSummary(delegations: Array<[string, boolean]>): string {
...
function enrichResponseWithArtifacts(content: string, filePaths: string[]): string {
  if (!filePaths.length) return content;
  const body = content || '';
  const missing = filePaths.filter((p) => !body.includes(p));
  if (!missing.length && filePaths.every((p) => body.includes(p))) return body;
...
export class BaseAgent {
  ...
  private skillConfigOverrides = new Map<string, Record<string, any>>();
  private baseSystemPrompt = '';
  private maxToolRounds: number;
  private maxToolRoundsHardCap = 40;
  private userTurnsSinceExtract = 0;
  private pendingExtracts = new Set<Promise<any>>();
  private pendingRequests = new Map<
    string,
    { resolve: (v: string) => void; reject: (e: Error) => void }
  >();
  private bgTasks = new Set<Promise<any>>();
  private readonly turnLock = new Mutex();
  private recallCache: Record<string, any[]> | null = null;
  ...
  constructor(
    ...
    const agentCfg = (
      config.agents as Record<string, { model: string | null; maxToolRounds?: number }>
    )[this.name];
  ...
  get name(): string {
    return (this.constructor as typeof BaseAgent).agentName;
  }
  get displayName(): string {
    return (this.constructor as typeof BaseAgent).agentDisplayName;
  }
  get emoji(): string {
    return (this.constructor as typeof BaseAgent).agentEmoji;
  }
  get specialty(): string {
    return (this.constructor as typeof BaseAgent).agentSpecialty;
  }
  get systemPrompt(): string {
    return (this.constructor as typeof BaseAgent).agentSystemPrompt;
  }
  get skillNames(): string[] {
    return (this.constructor as typeof BaseAgent).agentSkillNames;
  }

  private resolveSystemPrompt(): string {
    const lang = this.config.llm.language ?? 'zh';
    if (lang === 'en') {
      const cls = this.constructor as typeof BaseAgent;
      if (cls.agentSystemPromptEn) return cls.agentSystemPromptEn;
    }
    return this.systemPrompt;
  }

  private injectWorkspaceInfo(prompt: string): string {
  ...
  private injectBehaviorRules(prompt: string): string {
    const lang = this.config.llm.language ?? 'zh';
    const rules =
      lang === 'en'
  ...
  private injectProgrammingWisdom(prompt: string): string {
    const lang = this.config.llm.language ?? 'zh';
    const wisdom =
      lang === 'en'
  ...
  async init(): Promise<void> {
    if (this.baseSystemPrompt) return;
    await this.memory.initDb();
    if (this.memory.getActiveSession() === null) {
      let resumed: string | null = null;
      if (process.env.WA_NO_RESUME !== '1') resumed = await this.memory.resumeLatestSession();
      if (resumed === null) await this.memory.createSession();
    }
    this.baseSystemPrompt = this.resolveSystemPrompt();
    this.baseSystemPrompt = this.injectWorkspaceInfo(this.baseSystemPrompt);
    this.baseSystemPrompt = this.injectBehaviorRules(this.baseSystemPrompt);
  ...
  private registerSkillTools(): void {
    ...
    this.toolRegistry.register(
      new Tool({
        name: 'list_skills',
        description:
          'List all available skills with their names and descriptions. Use this first to discover what skills you can activate.',
        parameters: [],
        handler: async () => {
    ...
    this.toolRegistry.register(
      new Tool({
        name: 'use_skill',
        description:
          'Activate a named skill to gain specialized capabilities (e.g. code_reviewer for code review, web_research for research). Call list_skills first to see available options.',
        parameters: [
          {
            name: 'name',
            type: 'string',
    ...
    this.toolRegistry.register(
      new Tool({
        name: 'extend_rounds',
        description:
          'Extend the tool-call budget for the current turn. Call this when you know you need more iterations to finish.',
        parameters: [
          {
            name: 'n',
            type: 'integer',
  ...
  activateSkill(name: string): boolean {
    ...
    if (skill.maxTokens !== null) overrides.max_tokens = skill.maxTokens;
    if (Object.keys(overrides).length) this.skillConfigOverrides.set(name, overrides);
    this.rebuildSystemPrompt();
    return true;
  }
  ...
  deactivateAllSkills(): void {
    for (const name of [...this.activeSkills]) this.deactivateSkill(name);
  }

  private autoActivateSkills(message: string): string[] {
    if (!message) return [];
    const lowered = message.toLowerCase();
    const candidates = [...this.skills];
    for (const s of this.skillRegistry.getSkills()) {
      if (!candidates.find((c) => c.name === s.name)) candidates.push(s);
    }
    const activated: string[] = [];
    for (const skill of candidates) {
      if (this.activeSkills.has(skill.name) || !skill.triggers.length) continue;
      if (skill.triggers.some((t) => t && lowered.includes(t.toLowerCase()))) {
        if (this.activateSkill(skill.name)) activated.push(skill.name);
      }
    }
    return activated;
  }

  private runtimeIdentityBlock(): string {
    let model = this.config.llm.defaultModel;
    try {
      const agentCfg = (this.config.agents as Record<string, { model: string | null }>)[this.name];
  ...
  private rebuildSystemPrompt(): void {
    ...
    if (this.activeSkills.size) {
      ...
      for (const name of [...this.activeSkills].sort()) {
        ...
        if (!s) continue;
        const sp: string[] = [];
        if (s.systemPrompt) sp.push(s.systemPrompt);
        const lang = this.config.llm.language ?? 'zh';
        ...
        if (s.resourceDir)
          sp.push(
            lang === 'en' ? `Resource directory: ${s.resourceDir}` : `资源目录: ${s.resourceDir}`,
          );
        parts.push(sp.join('\n\n'));
      }
      prompt += '\n\n' + parts.join('\n\n');
  ...
  getSkillConfigOverrides(): Record<string, any> {
    ...
    for (const overrides of this.skillConfigOverrides.values()) Object.assign(merged, overrides);
    return merged;
  }
  ...
  private async handleEvent(event: Event): Promise<void> {
    if (event.type === EventType.TASK_ASSIGNED && event.target === this.name) {
      ...
      await this.bus.publish(
        makeEvent(EventType.TASK_COMPLETED, this.name, {
          target: event.source,
  ...
  async chatOneshot(
    prompt: string,
    opts: { model?: string | null; temperature?: number | null; maxTokens?: number | null } = {},
  ): Promise<string> {
    const overrides: Record<string, any> = {};
    if (opts.model !== undefined && opts.model !== null) overrides.model = opts.model;
    else {
      const lw = this.config.llm.lightweightModel;
      if (typeof lw === 'string' && lw.trim()) overrides.model = lw;
    }
  ...
  async chat(message: string, onStatus?: ((s: string) => void) | null): Promise<string> {
  ...
  async *chatStream(message: string): AsyncGenerator<ChatStreamEvent> {
  ...
  private async *chatStreamImpl(
    ...
    if (autoActivated.length) {
      ...
      this.memory.addMessage(
        'system',
        `[Auto-activated skills: ${autoActivated.join(', ')}] These were chosen from your message's keywords; their prompts and tools are already loaded. Do NOT call list_skills — proceed directly with the task. Only call use_skill(name) if you need a DIFFERENT skill that wasn't auto-activated.`,
      );
    }

    let recentToolOutcomes: boolean[] = [];
    let stuckHintInjected = false;
    let recentResponseTexts: string[] = [];
    let repetitionHintInjected = false;
    let recentToolSigs: string[] = [];
    let toolLoopHintInjected = false;

    let toolNamesCache: string[] | null = null;
    let cacheKey: [string, string] | null = null;
    ...
    const resolveToolNames = (): string[] => {
      ...
      const must = new Set(
        this.skills.filter((s) => this.activeSkills.has(s.name)).flatMap((s) => s.requiredTools),
      );
    ...
    try {
      ...
      while (roundCount < MAX_ROUNDS) {
        if (roundCount >= roundLimit) {
          if (roundLimit >= this.maxToolRoundsHardCap) break;
          const extendBy = Math.min(15, this.maxToolRoundsHardCap - roundLimit);
          roundLimit += extendBy;
          this.maxToolRounds = roundLimit;
          this.memory.addMessage(
            'system',
            `[Auto-extended tool-round limit by ${extendBy} to ${roundLimit}. Continue working.]`,
          );
          continue;
        }
        roundCount += 1;
        ...
        let streamUsage: Record<string, any> | null = null;
        let roundContent = '';
        ...
        if (streamUsage)
          this.bus.addEvent(
            makeEvent(EventType.LLM_CALL, this.name, { data: { model: '', usage: streamUsage } }),
          );
        ...
        for (const tc of toolCallsReceived) {
          ...
          const parseError =
            typeof rawArgs === 'string' && toolArgs === null
              ? formatArgsParseError(toolName, rawArgs)
              : null;

          this.bus.addEvent(
            makeEvent(EventType.TOOL_CALL, this.name, {
        ...
        const execOne = async (p: any): Promise<any> => {
          if (p.toolArgs === null)
        ...
        if (recentToolSigs.length) {
          ...
          for (const s of recentToolSigs) counter.set(s, (counter.get(s) ?? 0) + 1);
          let topSig = '',
            topCount = 0;
          ...
          if (topCount >= SIG_LOOP_HINT && !toolLoopHintInjected) {
            this.memory.addMessage(
              'system',
              `[Tool loop] You have called \`${topSig}\` ${topCount} times in the last ${recentToolSigs.length} tool calls — you are iterating on the same operation without converging. STOP repeating it. Either: (1) accept the current state and write the final answer, (2) try a fundamentally different approach (different tool / different target), or (3) call task_done if the work is good enough.`,
            );
          ...
          if (topCount >= SIG_LOOP_HARDSTOP) {
            this.memory.addMessage(
              'assistant',
              `I have repeated \`${topSig}\` ${topCount} times without converging on a better outcome. Stopping here to avoid burning the rest of the round budget on the same micro-adjustment. Current artifacts (if any) are the final state.`,
            );
            yield {
              type: 'content',
        ...
        for (const r of execResults) {
          if (r.toolName === 'task_done') continue;
          const failed =
            !r.success || (typeof r.result === 'string' && looksLikeFailedToolResult(r.result));
        ...
        if (recentToolOutcomes.length >= 8 && recentToolOutcomes.every((v) => !v)) {
          this.memory.addMessage(
            'assistant',
            "I tried multiple approaches but every tool call is failing (the file/path may not exist, search backends are blocked, or the resource is unavailable). Please give me more context — a correct file path, an alternative source, or clarification on what you're looking for.",
          );
          yield {
            type: 'content',
        ...
        const searchStorm = recentToolSigs.filter(
          (s) => s.startsWith('web_search:') || s === 'fetch_page' || s === 'http_get',
        ).length;
        if (searchStorm >= 8 && !toolLoopHintInjected) {
          this.memory.addMessage(
            'system',
            `[Search storm] You have made ${searchStorm} web-search / fetch calls in the last few rounds — far more than a productive research session needs. The queries are not converging on an answer. STOP searching. Synthesize the best answer you can from what you already know, label any gaps clearly, and call task_done. Do NOT make another web_search or fetch_page call.`,
          );
          toolLoopHintInjected = true;
        }
        if (searchStorm >= 12) {
          this.memory.addMessage(
            'assistant',
            'I have made too many search requests without finding the information. I will synthesize the best answer from available context.',
          );
          yield {
            type: 'content',
  ...
  private popLastUserMessage(): void {
  ...
  async compact(keepRecent = 12): Promise<string> {
    ...
    for (const m of toSummarize) {
      ...
      if (directiveKeywords.some((k) => c.toLowerCase().includes(k))) directives.push(c);
    }
  ...
  contextUsage(): {
    ...
    const model = (this.llm as any)._getModel
      ? (this.llm as any)._getModel(this.name)
  ...
  private activeToolNames(): string[] {
    ...
    for (const skill of this.skills) {
      ...
      if (skill.allowedTools === null) anyUnrestricted = true;
      else {
        if (restriction === null) restriction = new Set();
        for (const t of skill.allowedTools) restriction.add(t);
      }
    }
    if (restriction !== null && !anyUnrestricted) names = names.filter((n) => restriction!.has(n));
    return names;
  }
  ...
  private maybeExtractFacts(): void {
    ...
    if (this.userTurnsSinceExtract < everyN) return;
  ...
  private parseExtractedFacts(content: string): Array<Record<string, any>> {
    const text = (content || '').trim();
    if (!text) return [];
    try {
      const d = JSON.parse(text);
      if (Array.isArray(d)) return d.filter((f: unknown) => f && typeof f === 'object');
    } catch {
      /* pass */
    }
  ...
  private async messagesWithRecall(): Promise<Array<Record<string, any>>> {
    const msgs = this.memory.getMessages();
    if (!msgs.length || process.env.WA_NO_RECALL === '1') return msgs;
    let lastUserIdx = -1;
    ...
    if (query.trim().length < 4) return msgs;

    if (!this.recallCache) this.recallCache = {};
  ...
  protected async llmLoop(
    ...
    while (true) {
      if (rounds >= limit) {
        if (limit >= this.maxToolRoundsHardCap) break;
        const extendBy = Math.min(15, this.maxToolRoundsHardCap - limit);
        limit += extendBy;
        this.maxToolRounds = limit;
        this.memory.addMessage(
          'system',
          `[Auto-extended tool-round limit by ${extendBy} to ${limit}. Continue working.]`,
        );
        continue;
      }
      rounds += 1;
      ...
      this.bus.addEvent(
        makeEvent(EventType.LLM_CALL, this.name, {
      ...
      for (const tc of response.toolCalls) {
        ...
        if (onStatus) onStatus(toolArgs ? toolStatusLabel(toolName, toolArgs) : `${toolName}...`);
  ...
  async executeTask(task: Task, onStatus?: ((s: string) => void) | null): Promise<TaskResult> {
    const release = await this.turnLock.acquire();
    try {
  ...
  private async executeTaskImpl(
    ...
    let prompt =
      'Complete this task NOW using your available tools. Then write the actual deliverable content in your final reply.\n\n' +
    ...
    if (task.metadata) {
      ...
      if (Object.keys(ctx).length) prompt += `\nContext: ${JSON.stringify(ctx)}`;
    }
  ...
  private async checkToolApproval(
    ...
    if (mode === 'interactive') {
      if (this.approvalCallback) return this.approvalCallback(toolName, toolArgs);
  ...
  async requestHelp(targetAgent: string, description: string, timeoutS = 60): Promise<string> {
    const correlationId = randomUUID();
    let res: (v: string) => void, rej: (e: Error) => void;
    ...
    await this.bus.publish(
      makeEvent(EventType.AGENT_REQUEST, this.name, {
        target: targetAgent,
  ...
  private async handleRequest(event: Event): Promise<void> {
    ...
    try {
      const result = await this.executeTask(task);
      await this.bus.publish(
        makeEvent(EventType.AGENT_RESPONSE, this.name, {
          target: source,
