...
export function classify(goal: string): Mode {
  ...
  if (greetHits >= 1 && text.length < 30 && multiHits === 0 && actionHits === 0) return 'direct';
  if (multiHits >= 2 || listHits >= 2 || enumHits >= 3) return 'orchestrate';
  if (text.length > 200 && multiHits >= 1) return 'orchestrate';
  if (hasCode && text.length > 150) return 'orchestrate';
  if (hasPath || hasUrl || actionHits >= 1) return 'single';
  if (text.length < 50 && !hasCode) {
    if (singleHits >= 1 || text.endsWith('?') || text.endsWith('？')) return 'direct';
    if (multiHits === 0) return 'direct';
  }
  return 'single';
}

/** Pick one agent by keyword routing. Falls back to rain. */
export function pickAgentForKey(goal: string, available: Set<string>): string {
