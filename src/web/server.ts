...
export async function runVoiceServer(
  ctx: SystemContext,
  opts: { host?: string; port?: number; agentName?: string } = {},
): Promise<void> {
    ...
    if (req.url === '/ws' && req.headers.upgrade?.toLowerCase() === 'websocket') {
      ...
      socket.on('data', async (chunk: Buffer) => {
        ...
        while (buffer.length >= 2) {
          ...
          if (opcode !== 0x1) break; // only text frames
          ...
          if (buffer.length < offset + (masked ? 4 : 0) + payloadLen) break;
          ...
          if (mask) {
            for (let i = 0; i < payload.length; i++) {
              const m = mask[i % 4];
              const p = payload[i];
              if (m !== undefined && p !== undefined) payload[i] = p ^ m;
            }
          }
          ...
          try {
            ...
            if (msg.type === 'speech' && msg.text) {
              ...
              sendWsFrame(
                socket,
                JSON.stringify({ type: 'agent_list', agents: list, current: agentName }),
              );
...
function sendWsFrame(socket: Socket, text: string): void {
