...
function center(text: string, width: number, vw = vwidth(text)): string {
...
export function printWelcome(model: string, activeAgent: string, workspacePath = ''): void {
  ...
  for (const name of names) {
    const cls = (AGENT_CLASSES as any)[name];
  ...
  const shortWs =
    workspacePath && workspacePath.length > 40 ? '…' + workspacePath.slice(-38) : workspacePath;
...
export function agentBanner(name: string): string {
  const color = AGENT_COLORS[name] ?? 'white';
  const cls = (AGENT_CLASSES as any)[name];
  const emoji = AGENT_EMOJI[name] ?? '';
  return (
    styler(color).bold(`${emoji} ${cls?.agentDisplayName ?? name}`) +
