...
import { parseTaskPlan, type TaskPlan } from '../core/schemas.js';
...
const JSON_BLOCK_PATTERNS: RegExp[] = [
  /```json\s*\n([\s\S]*?)\n```/,
  /```\s*\n(\{[\s\S]*?\})\n```/,
];
...
- 执行:按优先级推进，完成后汇总`;
  ...
  static override agentSystemPromptEn = `You are "Snow" of Weather Agents.
...
Like snow — silent but all-covering. Clear structure, thorough consideration.`;
  ...
  async replanForMissing(
    goal: string,
    priorResults: Array<{ id: string; agent: string; success: boolean; content: string }>,
    missing: string,
    existingIds: Set<string> = new Set(),
  ): Promise<Task[]> {
    const usedIds = new Set(existingIds);
    const priorLines = priorResults.map(
      (r) =>
        `- task ${r.id} (${r.agent}, ${r.success ? '成功' : '失败'}): ${(r.content || '').slice(0, 200)}`,
    );
    ...
    const prompt =
      `之前的子任务执行后，验收员发现还有缺口。请仅针对**缺失的部分**追加新的子任务，并且**必须换一种执行策略**。\n\n` +
      `## 原目标\n${goal}\n\n` +
      `## 已执行子任务\n${priorText}\n\n` +
      `## 缺口（验收员报告）\n${missing}\n\n` +
  ...
  private schemaToTasks(plan: TaskPlan, goal: string): Task[] {
  ...
  private parseTaskPlanHeuristic(content: string, goal: string): Task[] {
  ...
  private static planToTasks(plan: any, goal: string): Task[] {
    const tasks: Task[] = [];
    for (const step of plan?.steps ?? []) {
      let agent = step.agent ?? 'rain';
      if (!VALID_AGENTS_STATIC.has(agent)) agent = 'rain';
